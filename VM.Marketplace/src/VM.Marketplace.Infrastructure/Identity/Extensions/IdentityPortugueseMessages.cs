...
namespace VM.Marketplace.Infrastructure.Identity.Extensions;
...
public class IdentityPortugueseMessages : IdentityErrorDescriber
{
    ...
    public override IdentityError ConcurrencyFailure() { return new IdentityError { Code = nameof(ConcurrencyFailure), Description = $"Falha de concorrência otimista, o objecto foi modificado!" }; }
    ...
    public override IdentityError InvalidEmail(string email) { return new IdentityError { Code = nameof(InvalidEmail), Description = $"Emai '{email}' é inválido!" }; }
    public override IdentityError DuplicateUserName(string userName) { return new IdentityError { Code = nameof(DuplicateUserName), Description = $"'{userName}' já está sendo utilizado!" }; }
    ...
    public override IdentityError UserAlreadyHasPassword() { return new IdentityError { Code = nameof(UserAlreadyHasPassword), Description = $"Úsuario já possui uma senha!" }; }
    ...
    public override IdentityError UserAlreadyInRole(string role) { return new IdentityError { Code = nameof(UserAlreadyInRole), Description = $"Úsuario já possui a permissão '{role}'!" }; }
    public override IdentityError UserNotInRole(string role) { return new IdentityError { Code = nameof(UserNotInRole), Description = $"Úsuario não tem a permissão '{role}'!" }; }
